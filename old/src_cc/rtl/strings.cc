...
LazSTRING itoa(int num, char* str, int base)
{
    int i = 0;
...
int atoi(char *str)
{
    ...
    // iterate through all charcters of input string
    // and update result ...
...
FPC_BEGIN
    ...
    WriteConsole(
        ...
        NULL);
FPC_END
static void WriteLnHelper(uint32_t s)
FPC_BEGIN
    check_console();
    
    long unsigned int i;
    char*     str = new char[100];
    LazSTRING sz  = itoa(s,str,10);
    uint32_t  len = strlen(strim(str));
...
LazVOID FPC_DLLFUNC(WriteLnString,rtl)(LazSTRING s)
FPC_BEGIN
	check_console();
	
	long unsigned int i;
	uint8_t len = strlen(strim(s));
...
LazVOID FPC_DLLFUNC(WriteLnByte,   rtl)(LazBYTE    s) FPC_BEGIN WriteLnHelper(s); FPC_END
LazVOID FPC_DLLFUNC(WriteLnChar,   rtl)(LazCHAR    s) FPC_BEGIN WriteLnHelper(s); FPC_END
LazVOID FPC_DLLFUNC(WriteLnWord,   rtl)(LazCHAR    s) FPC_BEGIN WriteLnHelper(s); FPC_END
LazVOID FPC_DLLFUNC(WriteLnInteger,rtl)(LazINTEGER s) FPC_BEGIN WriteLnHelper(s); FPC_END
...
// ----------------------------------------------------------
// get the length of an Word array.
// ----------------------------------------------------------
LazINTEGER FPC_DLLFUNC(LengthWord,rtl)(LazINTEGER s)
FPC_BEGIN
...
// ----------------------------------------------------------
LazINTEGER FPC_DLLFUNC(LengthString,rtl)(LazSTRING s)
FPC_BEGIN
